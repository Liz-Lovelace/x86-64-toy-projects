...
_start:
    ...
    mov r10, 0
    .notes_reading_loop:
        mov r14b, [raw_track + r10] ; load note number (0 - 128)

        cmp r14b, 0x0A ; if current note number is \LF
        je exit        ; then quit

        movzx r14, r14b ; apparently we can't use r14b for addressing into notes_is_playing without padding it with zeroes
        mov byte [notes_is_playing + r14], 0xff ; turn the note on
        ...
        .play_for_a_bit:
            ...
            inc r12
            cmp r12, 30000 ; this will be replaced by a dynamic wait time for a certain number of samples
            jne .play_for_a_bit  ; this loop seems to be going forever

        ; mov byte [notes_is_playing + r14], 0x00 ; turn the note off
...
sum_all_notes: 
    ...
    .loop:
        ...
        sar r13w, 2     ; divide by 4 to make every individual note 4 times quieter. This is so that things don't clip. Better system would be nice
        add word ax, r13w
